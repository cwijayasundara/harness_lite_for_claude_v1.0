...
const ALLOWED = {
  intent: new Set(['draft', 'approved', 'closed']),
...
function approvalCommit(root, artifact) {
  ...
  for (const commit of commits) {
    ...
    if (field(body, 'Status') === 'approved') return git(root, ['show', '-s', '--format=%aI', commit]) || null;
  }
  return null;
}
...
function activeStage(a) {
  ...
  if (!a.intent.approved_at) return 'intent-approval';
...
export function lifecycle(cfg, onlySlug = null, now = Date.now()) {
  ...
  return (onlySlug ? [onlySlug] : allSlugs(cfg.layout)).map((slug) => {
    ...
    for (const kind of KINDS) {
      ...
      if (item?.status === 'approved' && !item.approved_at) issues.push(`${kind}: approval is not committed`);
    }
    for (let i = 1; i < KINDS.length; i++) {
      const current = artifacts[KINDS[i]]; const previous = artifacts[KINDS[i - 1]];
      if (current && previous?.status !== 'approved') issues.push(`${KINDS[i]} exists before ${KINDS[i - 1]} approval`);
