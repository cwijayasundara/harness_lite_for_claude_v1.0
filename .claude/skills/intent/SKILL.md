...
1. `bash .claude/bin/harness new intent <slug>` — creates the file from the template.
...
5. Stop. Ask the person to review it. Do not write a spec yet.

## When the source is a PRD

Read it, then still interview. A PRD tells you what someone wants; it rarely tells you what is
wrong today, and the problem statement is the part the spec is checked against later.
