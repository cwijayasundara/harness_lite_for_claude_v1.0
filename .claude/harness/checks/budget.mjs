...
const PROVIDED = path.dirname(HARNESS); // <root>/.claude, the parent of the harness
...
export const shipped = (root) => {
  const canonical = existsSync(path.join(root, '.claude/harness')) ? path.join(root, '.claude/harness') : root;
...
export function measure(cfg) {
  ...
  // The harness hook surface is canonical under .claude/harness. Agent-specific projections are generated
  ...
  const external = self ? { skills: 0, agents: 0 } : recorded(cfg.layout.harness);
...
export async function run(cfg) {
  ...
  for (const [k, v] of Object.entries(m)) {
    ...
    if (v === null) {
      findings.push({ file: `.claude/harness/${RECORD}`, line: 0, rule: `budget/${k}`,
        message: `${k} could not be accounted for — the budget measured nothing and will not report a pass`,
        fix: `re-run 'harness init --into .' to regenerate .claude/harness/${RECORD}` });
    } else if (max != null && v > max) {
      findings.push({ file: '.claude/harness/harness.toml', line: 0, rule: `budget/${k}`,
