...
//
// The control plane targets Claude Code: `.claude/harness/instructions.md`, markdown skills, hook
// intents in `hooks/policy.json` and a CLI with exit codes. The agent-neutral split that used to
// live here — a provider-independent kernel in its own top-level directory, with per-agent
// adapters projecting out of it — was retired when the harness moved under `.claude/`: it cost
// an indirection and bought a runtime nobody had built a feature through.
...
export function renderClaudeHooks(policy, commandRoot = '${CLAUDE_PLUGIN_ROOT}') {
  ...
  const matchers = { write: 'Write|Edit|MultiEdit', shell: 'Bash', safety: 'Write|Edit|MultiEdit|Bash' };
  for (const binding of policy.bindings) {
    ...
    const entry = { hooks: [{ type: 'command', command: `node "${commandRoot}/.claude/harness/bin/harness" hook ${binding.action}`, timeout: binding.timeout }] };
    if (binding.matcher) {
  ...
  return { description: 'Generated Claude hook projection of .claude/harness/hooks/policy.json.', hooks };
}
...
export function renderClaudeInstructions(source) {
  return `<!-- Generated from .claude/harness/instructions.md; edit the canonical file and run harness init. -->\n${source.trim()}\n`;
}

// MEASURED 2026-09-16, M1 step 2: `harness init` into a repository that already had a CLAUDE.md
// adopted that file verbatim as `.claude/harness/instructions.md` and never opened
...
//
// MEASURED, G24 pilot on `calculator` (.claude/harness/evals/comparisons/2026-09-16T08-15-39-145Z): the
...
//
// `.claude/harness/state/` is already gitignored, but a formatter reads the working tree, not git.
