...
// fully-run set of results; `.claude/harness/checks/baseline.mjs` calls it a second time with the subset
...
export function render(report, layoutPaths) {
  ...
  for (const c of report.controls) {
    ...
    if (c.truncated > 0) lines.push(`      ... ${c.truncated} more, full report: ${layoutPaths?.lastCheck ?? '.claude/harness/state/last-check.json'}`);
  }
  if (!report.ok) {
    lines.push('');
