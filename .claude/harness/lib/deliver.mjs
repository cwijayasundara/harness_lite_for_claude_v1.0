...
// (.claude/harness/evals/comparisons/2026-09-16T10-51-24-499Z): this counted `## Blocking` as a FINDING
...
export async function deliver(cfg, slug, {
  ...
  if (previous && previous.plan_digest !== digest) {
    throw new Error(`${slug}'s approved plan changed since the interrupted run (recorded ${previous.plan_digest}, now ${digest}). ` +
      'The authority that run was executing under is gone. Delete .claude/harness/state/deliver/' + slug + '/phases.json to start a fresh run against the new plan.');
  }
  ...
  const contract = `Change: ${slug}\nSpec: .claude/harness/artifacts/${slug}/spec.md\nPlan: .claude/harness/artifacts/${slug}/plan.md\n` +
