...
function artifactOrState(rel) {
  return rel.startsWith('.claude/harness/artifacts/') || rel.startsWith('.claude/harness/state/');
}
...
export function writeRefusal(rel, cfg) {
  ...
  // `.claude/harness/harness.toml` in this session's prompt prefix is the one at the root. The suffix
  // match also caught every nested copy — it refused an edit to
  // `evals/fixtures/_base/.claude/harness/harness.toml`, a fixture that is never read into any prompt.
