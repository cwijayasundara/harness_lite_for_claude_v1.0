...
const ARTIFACTS = '.claude/harness/artifacts/';
const artifactPath = slug => `${ARTIFACTS}${slug}`;
// Derived from the prefix, never a fixed segment index: the harness directory has moved once
// already, and a hard-coded [2] silently parsed the slug as 'artifacts' when it did.
const slugOf = p => p.slice(ARTIFACTS.length).split('/')[0];
...
function delivery(root, revision, catalog, record, c) {
  ...
  const changed = changedFiles({ layout: { root }, diff: { base: record.base, candidate: record.candidate } }).filter(p => !p.startsWith('.claude/harness/artifacts/'));
...
function catalogRecords(root, catalog) {
  const commits = git(root, ['rev-list', '--full-history', `--max-count=${MAX_HISTORY + 1}`, catalog, '--', '.claude/harness/artifacts']).trim().split('\n').filter(Boolean);
  ...
  for (const sha of [catalog, ...commits.filter(c => c !== catalog)]) {
    for (const e of entries(root, sha).filter(e => /^\.claude\/harness\/artifacts\/[^/]+\/delivery\.json$/.test(e.path))) {
...
export function productContext(cfg, { revision, records = 'HEAD' } = {}) {
  ...
  try {
    ...
    for (const e of entries(root, catalog).filter(e => /^\.claude\/harness\/artifacts\/[^/]+\/spec\.md$/.test(e.path))) {
