...
export function identityTools(fs, path, crypto, cp) {
  const roots = ['.claude/harness/bin', '.claude/harness/lib', '.claude/harness/checks', '.claude/harness/sensors', '.claude/harness/hooks', '.claude/harness/hooks.json', '.claude/harness/skills', '.claude/harness/roles', '.claude/harness/templates', '.claude/harness/policies', '.claude/harness/instructions.md', '.claude-plugin'];
  ...
  function manifest(root) {
    ...
    if (!entries.some(e => e.path === '.claude/harness/bin/harness')) throw new Error('runtime entrypoint missing');
  ...
  function verify(project, root, self = false) {
    ...
    let record;
    try { const data = safeRead(project, '.claude/harness/harness-install.json'); record = JSON.parse(data.bytes.toString('utf8')); } catch { /* explicit unavailable below */ }
...
// The repository root, which is what the identity roots are relative to. Three levels up:
// this file is <root>/.claude/harness/lib/runtime-identity.mjs.
...
export function policyIdentity(root) {
  const paths = ['.claude/harness/harness.toml', '.claude/harness/instructions.md', '.claude/harness/policies/review.md', '.claude/CLAUDE.md', '.claude/settings.json', '.claude/settings.local.json', '.mcp.json', 'CLAUDE.md', 'AGENTS.md'];
  try {
...
const project = path.dirname(path.dirname(process.env.HARNESS_PROJECT_AIDLC));
let rec; try { rec = JSON.parse(api.safeRead(project, ".claude/harness/harness-install.json").bytes.toString("utf8")); } catch { rec = {}; }
const home = process.env.HARNESS_HOME;
const rootOf = d => path.basename(d) === "harness" && path.basename(path.dirname(d)) === ".claude" ? path.dirname(path.dirname(d)) : d;
const attempt = d => { const root = rootOf(d); const result = api.verify(project, root); if (result.status === "verified") { process.stdout.write(path.join(root, ".claude/harness")); return true; } process.stderr.write("harness: runtime " + result.status + " expected=" + (result.expected?.commit ?? rec.commit ?? "unknown") + " observed=" + (result.observed?.commit ?? "unavailable") + ". " + result.remedy + "\\n"); return false; };
if (home) { if (attempt(home)) process.exit(0); process.stderr.write("harness: HARNESS_HOME mismatch or holds no .claude/harness/bin/harness; no fallback.\\n"); }
