# lean-harness

A harness for Claude Code: guides, sensors, and the ledger that decides which survive.
This repository builds the harness and is self-exempt: its Claude plugin and project hooks stay
disabled here to avoid recursively running the harness while changing it.

## AIDLC workflow

Users start or resume in natural language: “Take this through the Lean AIDLC workflow” or
“Approved. Continue the workflow.” Run `harness status`, then resume the first incomplete stage:

`intent -> spec (gate 1) -> plan (gate 2) -> implement -> review -> merge (gate 3)`

Select the intended change with `harness status --change <slug>` in each worktree before
implementation. Selection is local and never approves a gate; backlog drafts do not block it.
After switching branches, explicitly reselect. Use `--clear-change` to end execution.

Invoke harness commands yourself; never make the user drive the workflow through the CLI. The
one exception is `approve`, which is theirs to run — the gate is the point.

## Agent commands

```
.claude/harness/bin/harness check --stage stop     # secrets + the full unit suite
.claude/harness/bin/harness check --stage commit   # + scope-drift, budget, arch, baseline
.claude/harness/bin/harness new <slug>             # intent.md, spec.md, plan.md, review.md
.claude/harness/bin/harness approve <slug> spec|plan --by <them>     # the human's gate, digested
.claude/harness/bin/harness status                 # where each change is, and whether its approvals hold
.claude/harness/bin/harness ledger audit           # which controls earn their place, and which rules misfire
.claude/harness/bin/harness ledger flag <rule>     # a fire the guard got wrong
.claude/harness/bin/harness graph query <question> # callers, calls, hubs, cycles, changed-since
.claude/harness/bin/harness deliver <slug> --live   # the seven delivery phases, bounded and resumable
.claude/harness/bin/harness pack <symbol>          # budgeted definition, callees, callers
.claude/harness/bin/harness evals gate             # grade the newest full run against evals/expected.json
```

Change assignments in the existing tracker and record the tracker URL; local assignee fields are
projections, not assignment authority.

## Finding your way around

Locate indexed symbols with `harness graph query` and `harness pack` first.
Grep/Glob is the miss path. G16 folded the `map` skill into this section — it was a whole skill to say "ask the index
before you grep", which is standing advice and not a procedure anyone invokes.

| You want to know | Command |
|---|---|
| Who would break if I change this | `harness graph query callers <symbol>` |
| What this depends on | `harness graph query calls <symbol>` |
| Where the load-bearing modules are | `harness graph query hubs` |
| Whether the structure has knots | `harness graph query cycles` |
| What this change has already touched | `harness graph query changed-since HEAD` |

`harness pack <symbol>` returns a budgeted definition with its callees and callers (~1200 tokens;
`--budget` to change that). `CODEBASE-MAP.md` is the written map — load-bearing modules ranked by
what depends on them, capped at 200 lines, generated by `harness map`. Read it once at the start of
unfamiliar work instead of opening files to find out what is where.

The graph is a cache, not an authority. It is line-oriented: module imports are reliable, call
edges are a heuristic filtered against the known symbol table. **A miss means grep, not "it does
not exist"** — never conclude nothing calls something from the graph alone. A `STALE since` banner
means the last refresh failed, so verify anything load-bearing against the source. `map-drift` in
the Stop output means `CODEBASE-MAP.md` no longer describes the tree: run `harness map` and commit
it. Do not read `.claude/harness/state/graph.json` directly; it is a cache format and it will change.

Inspect a revision with `git show <rev>:<path>` and `git grep`. Use `harness graph query product
--revision <commit>` only when delivery records already exist; unknown coverage and unresolved
links are findings, not negative proof.

Never report a task complete without running and pasting the output of `--stage stop` yourself.

## Conventions

- **Zero dependencies.** No `node_modules`, ever. The suite must run on a cold clone. If a
  problem seems to need a library, it is the wrong problem — see the tree-sitter decision in
  `docs/BUILD-PLAN.md` Phase 3.
- **The budget is full.** `[limits]` in `.claude/harness/harness.toml` states the ceilings and the harness
  sits at them. Adding one means deleting one. `--stage commit` enforces this; do not raise a limit to get past it.
- **Law 11**: a control enters only with a failing eval, or a defect recorded while building a
  non-harness application through the harness. A defect in harness machinery earns a fix, not a
  new control. Both previous attempts grew because the only workload they governed was themselves.
- Control flow goes in `.claude/harness/bin/harness`, never into a SKILL.md. A numbered sequence longer than
  eight steps in a skill fails `test/contracts.test.mjs`.
- `.claude/harness/harness.toml` is the only hand-edited registry. Claude settings are generated from
  `.claude/harness/hooks/policy.json` by `harness init`—never edit them by hand.
- Every control carries a `why:` naming the defect it prevents. No why, no control.
- Tests are `node:test`, zero-dep, and live in `test/`. Fixtures under `evals/fixtures/` are
  write-protected: a fixture edited to make a test pass is a fixture that no longer tests.
- A guard that refuses a path an approved committed contract owns is a gate inside the build
  loop. Ownership is the human's decision; the guards yield to it and to nothing else.

## The artifact chain

One directory per change, `.claude/harness/artifacts/<slug>/`:

- `intent.md` — problem, outcome, affected users and systems, constraints, open questions.
- `spec.md` — numbered `### B<n>` behaviours, consequential design, boundaries and safeguards. **Gate 1.**
- `plan.md` — approach, `## Files`, order, and a proof row per behaviour. **Gate 2.**
- `review.md` — evaluator findings citing behaviours or review passes, evidence and uncertainty.

Approval metadata records a decision; its digest detects edits, not human identity. Editing reports
`stale-approval` rather than silently still reading as approved. `## Files` in the plan is the
only declaration of ownership: `scope-drift` and the write guard read it and nothing else. The
final gate is human PR approval and merge.

Read the relevant system and existing patterns before changing it. Ask only consequential
unresolved questions; do not invent questions or split a coherent outcome on the word "and".
Work in small behavioural slices, reproduce defects before fixing them, and verify runtime
behaviour. Legitimate test maintenance is allowed when it preserves proof of approved behaviour;
weakening assertions to hide failures is not. Material design or scope changes return to the
human. Routine choices inside current approved boundaries proceed without another gate.

## Generator and evaluator

`[models]` names three. `generator` (Sonnet 5) writes the code, through the `implement` skill in a
forked context. `harness review --base <commit> --candidate <commit> --out <file>` invokes the
evaluator on explicit snapshots with only Read/Grep/Glob. The caller saves findings and runs
checks separately. `evals` names the golden-suite model; capable-model product trials are separate.

`harness init` renders the first two from the registry, and a test fails if generator and
evaluator are ever the same id. A changes-requested review returns to `implement` at most twice,
then the human decides.

Deploy and Maintain are project-owned. The harness ships one example,
`examples/maintain/band-to-intent.mjs`, which turns a control-band breach into an intent.
