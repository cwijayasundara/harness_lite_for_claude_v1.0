...
import { deliver, PHASES } from '../.aidlc/lib/deliver.mjs';
...
test('a breach against a committed bands file writes an intent the gate can bind', () => {
  ...
  try {
    ...
    const intent = readFileSync(path.join(p.root, '.aidlc/artifacts', SLUG, 'intent.md'), 'utf8');
    ...
    const pending = path.join(p.root, '.aidlc/evals/pending', `${SLUG}.json`);
...
test('an unbindable bands document is reported as unbound, never written as a half-declaration', () => {
  ...
  try {
    ...
    assert.equal(result.status, 0, result.stderr);
    const { front } = a.parse(readFileSync(path.join(uncommitted.root, '.aidlc/artifacts', SLUG, 'intent.md'), 'utf8'));
...
test('breach to intent to approved spec to the driver, in one path and with no model', async () => {
  ...
  try {
    ...
    const cfg = p.cfg();
    const dir = path.join(p.root, '.aidlc/artifacts', SLUG);
...
test('a quiet band writes nothing, and a human edit to a breach intent survives the next run', () => {
  ...
  try {
    ...
    assert.deepEqual(readdirSync(path.join(p.root, '.aidlc/artifacts')), []);
    ...
    assert.equal(detect(p.root, 'bands.json').status, 0);
    const intent = path.join(p.root, '.aidlc/artifacts', SLUG, 'intent.md');
