...
export async function run(cfg) {
  ...
  const inFlight = artifact.kind === 'contract' ? ownedFiles(body) : declaredFiles(body);
  if (!inFlight?.length) return { verdict: 'fail', findings: [{ file: artifact.rel, line: 0, rule: 'delivery-scope-missing', message: `${artifact.f} declares no owned files`, fix: artifact.kind === 'contract' ? 'add exact paths under "## Structure and ownership"' : 'add a fenced "## Files" block' }] };
