...
import { diff as readDiff, changedStatuses, unbornRepository } from '../lib/diff.mjs';
...
export async function run(cfg) {
  if (unbornRepository(cfg)) return { verdict: 'skipped', findings: [], note: 'no commits yet; local tamper requires HEAD' };
  const statuses = changedStatuses(cfg);
  ...
  for (const { status, file: name } of statuses) {
